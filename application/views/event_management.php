...
<div class="page-body container">
    ...
    <div class="global-search-filter-container">
        ...
        <div class="mt-5 text-center">
            <div class="mt-2 text-center">
                <button type="button" class="btn btn-primary" id="getvaluationbyproject">Make My Movie</button>
            </div>
        </div>
        ...
        <div class="modal-overlay" id="valuationModel"
            ...
            <div class="modal-box"
                ...
                <div class="d-flex">
                    ...
                    <h3 class="" style="margin-left: 400px;">Total Events: <?= $this->CommonModal->countRowsByCondition('calendar_timeline_master',[
                        'user_id' => sessionId('freelancer_id')
                    ]) ?></h3>
                </div>
                <div class="card">
                    <div class="createNameContainer" style="display:none;">
                    <form action="<?php echo base_url('/company/create_my_movie_name') ?>" method="post"
                        id="create_makemymovie">
                        <div class="d-flex">
                            <div class="form-group" style="width:80%;">
                                <label> Create Name</label>
                                <input type="text" placeholder="Title" value="My Movie 1" id="makemymoviename"
                                    name="makemymoviename" class="form-control" required>
                            </div>
                    ...
                    <div class="createdMovieName" id="createdmoviename"></div>
                    ...
                    <div
                        ...
                        <div>
                            <div class="d-flex align-items-center">
                                <h3 id="modalMovieName" class="mb-0"></h3>
                                <a id="playMovieBtn" class="btn btn-primary btn-md ms-2">
                                    <i>Play</i>
                                    <i class="fa-solid fa-play"></i>
                                </a>
                                <h3 class="" style="margin-left: 380px;">Total Events: <?= $this->CommonModal->countRowsByCondition('calendar_timeline_master',[
                                    'user_id' => sessionId('freelancer_id')
                                ]) ?></h3>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-12 col-md-6 col-lg-4">
                                    <form method="GET"
                                        action="<?php echo base_url('/home/get_calendar_timelines_view') ?>"
                                        class="global-search-filters" style="display:flex;" id="globalSearchForm">
                                        <div class="filter-box mt-5" style="width:90%;">
                                            <div style="text-align: left !important;"> Search 
                                                <select id="search_type_select" class="" aria-label="Default select example">
                            ...
                            <form action="<?php echo base_url('/company/create_event_tree') ?>" method="post"
                                id="create_event_tree">
                                <input type="hidden" class="form-control" id="modalMovieId">
                            ...
                            <div id="alreadyEventsResult" style="margin-top:100px;"></div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#create_makemymovie').on('submit', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            if (response.getmakemymoviename && response.getmakemymoviename.length > 0) {
                                const movies = response.getmakemymoviename;
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                <a href="#"
                                   class="openMovieModal"
                                   data-id="${movie.id}"
                                   data-name="${movie.makemymoviename}">
                                   ${index + 1}. ${movie.makemymoviename}
                                </a>
                                <span class="badge bg-primary rounded-pill">
                                    ${movie.total_events || 0} Events
                                </span>
                            </li>`;
                                });
                                html += '</ul>';
                                ...
                                $('#createdmoviename').html(html);

                                // Store last created book ID in hidden input
                                const lastMovie = movies[movies.length - 1];
                                $('#created_movie_id').val(lastMovie.id);
                            }
            ...
            $('#getvaluationbyproject').on('click', function (e) {
                ...
                $.ajax({
                    url: "<?php echo base_url('company/get_all_make_my_movies') ?>",
                    method: 'POST',
                    dataType: 'json',
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                $('#createdmoviename').html('<p>No movies found.</p>');
                            }

                        } else {
                            $('#createdmoviename').html('<p class="text-center mt-5">No movies found.</p>');
            ...
            $(document).on('click', '.openMovieModal', function (e) {
                e.preventDefault();
                ...
                $.ajax({
                    url: "<?php echo base_url('company/get_events_by_movie_id') ?>",
                    type: 'POST',
                    data: {
                        movie_id: movieId
                    },
                    dataType: 'json',
                    beforeSend: function () {
                        $('#alreadyEventsResult').html('<p>Loading bricks...</p>');
                    },
                    success: function (response) {
                        if (response.success) {
                            console.log('response', response);
                            $('#alreadyEventsResult').html(response.html);
                            let modal = $('#movieModal');
                            // console.log('modal', modal);
                            initSteps(modal);
                            let playMovieBtn = modal.find('#playMovieBtn');
                            playMovieBtn.attr("href","<?= base_url('Home/movie_pdf') ?>" + `?id=${movieId}`)
                        } else {
                            $('#alreadyEventsResult').html('<p style="color:red;">' + (response.message || 'Something went wrong.') + '</p>');
                        }
                    },
                    error: function (xhr, status, error) {
                        console.error(error);
                        $('#alreadyEventsResult').html('<p style="color:red;">AJAX Error: ' + error + '</p>');
                    }
                });
            });
    ...
    <script>
        $(document).ready(function () {
            $('#globalSearchForm').on('submit', function (e) {
                ...
                let searchType = $('#search_type_select').val() || '';

                if (searchType.trim() === "") {
                    $('#searchResult').html('<p style="color:red;">Please Select Search Type.</p>');
                    return;
                }
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#create_event_tree').on('submit', function (e) {
                ...
                let selectedEvents = [];
                $('input[name="mymovieevent[]"]:checked').each(function () {
                    selectedEvents.push($(this).val());
                });
                console.log('selectedEvents',selectedEvents)
                if (selectedEvents.length === 0) {
                    alert('Please select at least one event before saving.');
                    return;
                }

                // Optional: collect modalBookId if you’re using it
                let modalMovieId = $('#modalMovieId').val();
                console.log('modalMovieId', modalMovieId)
                $.ajax({
                    url: actionUrl,
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        mymovieevents: selectedEvents,
                    ...
                    success: function (response) {
                        if (response.status === 'success') {
                            ...
                            $('input[name="mymovieevent[]"]').prop('checked', false);
                        } else {
                    ...
                    error: function (xhr, status, error) {
                        console.error(error);
                        alert('Error while saving events.');
                    },
                    complete: function () {
