    ...
    <div class="team-members-wrapper">
        <?php if ($bricks['user_id'] == sessionId('freelancer_id')) { ?>
            <div class="viewpermissiontocompanyteam">
                <!-- Question 1 -->
                <div>
                    <form id="brickCompanyPermissionForm" method="post">
                        <dl class="d-flex align-items-center gap-2" id="questionBox1">
                            ...
                            <dd class="mb-0 d-flex align-items-center gap-3">
                                <label class="switch me-2">
                                    <input type="checkbox" id="brick_company_toggle" <?= !empty($brickCompanyPerm) ? 'checked' : '' ?>>
                                    <span class="slider round"></span>
                                </label>
                                <span id="brick_company_label">No</span>
                            </dd>

                            <input type="hidden" value="<?= $brick_id ?>" id="brick_id_company">
                            <input type="hidden" value="<?= $bricks['company_id'] ?>" id="company_id_company">

                            <div class="d-flex gap-2 view-selector" id="brick_company_views">
                            ...
                            <input type="hidden" id="brick_company_selected_view">

                            <select class="form-select ms-3" style="width:200px;" id="brick_company_permission" required>
                                <option value="viewer" <?= ($brickCompanyPerm['permission'] ?? '')=='viewer'?'selected':'' ?>>Viewer</option>
                                <option value="editor" <?= ($brickCompanyPerm['permission'] ?? '')=='editor'?'selected':'' ?>>Editor</option>
                                <option value="comment" <?= ($brickCompanyPerm['permission'] ?? '')=='comment'?'selected':'' ?>>Comment</option>
                            </select>
                ...
                <!-- Question 2 -->
                <div>
                    <form id="brickProjectPermissionForm" method="post">
                        <dl class="d-flex align-items-center gap-2" id="questionBox2">
                            <dt class="small me-2 mb-0">You want Project Members to be part of this brick ?</dt>

                            <dd class="mb-0 d-flex align-items-center gap-3" style="margin-left: 17px;">
                                <label class="switch me-2">
                                    <input type="checkbox" id="brick_project_toggle" <?= !empty($brickProjectPerm) ? 'checked' : '' ?>>
                                    <span class="slider round"></span>
                                </label>
                                <span id="brick_project_label">No</span>
                            </dd>

                            <input type="hidden" value="<?= $brick_id ?>" id="brick_id_project">
                            <input type="hidden" value="<?= $bricks['project_id'] ?>" id="project_id_project">

                            <div class="d-flex gap-2 view-selector" id="brick_project_views">
                                <span class="view-btn px-2 py-1 border border-secondary" data-value="1d">1D View</span>
                                <span class="view-btn px-2 py-1 border border-secondary" data-value="2d">2D View</span>
                                <span class="view-btn px-2 py-1 border border-secondary" data-value="3d">3D View</span>
                                <span class="view-btn px-2 py-1 border border-secondary" data-value="family">Artificial Family</span>
                            ...
                            <input type="hidden" id="brick_project_selected_view">

                            <select class="form-select ms-3" style="width:200px;" id="brick_project_permission" required>
                                <option value="viewer" <?= ($brickProjectPerm['permission'] ?? '')=='viewer'?'selected':'' ?>>Viewer</option>
                                <option value="editor" <?= ($brickProjectPerm['permission'] ?? '')=='editor'?'selected':'' ?>>Editor</option>
                                <option value="comment" <?= ($brickProjectPerm['permission'] ?? '')=='comment'?'selected':'' ?>>Comment</option>
                            </select>
...
<script>
$(document).on('click', '#brick_company_views .view-btn', function () {
    $('#brick_company_views .view-btn').removeClass('active');
    $(this).addClass('active');
    $('#brick_company_selected_view').val($(this).data('value'));
});

$(document).on('click', '#brick_project_views .view-btn', function () {
    $('#brick_project_views .view-btn').removeClass('active');
    $(this).addClass('active');
    $('#brick_project_selected_view').val($(this).data('value'));
});
</script>

<script>
$('#brickCompanyPermissionForm').on('submit', function (e) {
    e.preventDefault();

    const brickId    = $('#brick_id_company').val();
    const enabled    = $('#brick_company_toggle').is(':checked') ? 1 : 0;
    const permission = $('#brick_company_permission').val();

    $.ajax({
        url: "<?= site_url('home/saveBrickPermission') ?>",
        type: "POST",
        dataType: "json",
        data: {
            entity_type: 'brick',
        ...
        success: function (res) {
            if (res.success) {
                $('#brick_company_label').text(enabled ? 'Yes' : 'No');
                alert('Company team permission updated ✅');
            } else {
...
$('#brickProjectPermissionForm').on('submit', function (e) {
    e.preventDefault();

    const brickId    = $('#brick_id_project').val();
    const enabled    = $('#brick_project_toggle').is(':checked') ? 1 : 0;
    const permission = $('#brick_project_permission').val();
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                $('#brick_project_label').text(enabled ? 'Yes' : 'No');
                alert('Project team permission updated ✅');
            } else {
                alert(res.message || 'Failed');
            }
        },
