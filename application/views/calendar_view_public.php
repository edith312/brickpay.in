    ...
    <div class="row align-items-start">
        ...
        <div class="col-md-9">
            ...
            <div id="calendarWrapper">
                ...
                <button class="btn-filter d-none">
...
<script>
    const calendarPermission = <?= json_encode($perm_details ?? null); ?>;
</script>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        let calendarConfig = {
            ...
            dayCellDidMount: function (info) {
                info.el.addEventListener("click", function () {
                    ...
                    // 🔒 Block clicks if outside allowed range
                    if (calendarPermission && !isDateAllowed(info.date)) {
                        return;
                    }

                    renderCalendarModal(info.date);
                });
            },
            ...
            datesSet: function(info) {
                ...
                try {
                    ...
                    // 🔒 Stop processing if outside allowed range
                    // if (calendarPermission && !isRangeAllowed(info.start, info.end)) {
                    //     return;
                    // }

                    loadEventCounts(info.startStr, info.endStr, calendarPermission.id);
                    loadCountsForRange(info.startStr, info.endStr, 'monthly', calendarPermission.id);

                    const viewDate = mainCalendar.getDate();
                    const year = viewDate.getFullYear();

                    loadCountsForRange(`${year}-01-01`, `${year}-12-31`, 'yearly', calendarPermission.id);
        ...
        /////////////////////////////////////////////////////////
        // 🔥 APPLY PERMISSION RESTRICTION HERE
        /////////////////////////////////////////////////////////

        if (calendarPermission) {

            const scope = calendarPermission.permission_scope;

            if (scope === 'day') {

                const allowedDate = calendarPermission.reference_date;

                calendarConfig.initialView = 'dayGridDay';
                calendarConfig.initialDate = allowedDate;

                calendarConfig.validRange = {
                    start: allowedDate,
                    end: allowedDate
                };
            ...
            if (scope === 'month') {
                ...
                const [y, m] = calendarPermission.reference_month.split('-');
            ...
            if (scope === 'year') {
                ...
                let year = calendarPermission.reference_year;

                calendarConfig.initialDate = year + '-01-01';
        ...
        const mainCalendar = new FullCalendar.Calendar(calendarEl, calendarConfig);
        mainCalendar.render();

        function isDateAllowed(dateObj) {

            if (!calendarPermission) return true;

            const scope = calendarPermission.permission_scope;

            // 🔹 Build local YYYY-MM-DD safely
            ...
            if (scope === 'day') {
                return localDateStr === calendarPermission.reference_date;
            }

            if (scope === 'month') {
                const localMonthStr = `${year}-${month}`;
                return localMonthStr === calendarPermission.reference_month;
            }

            if (scope === 'year') {
                return String(year) === String(calendarPermission.reference_year);
            }

            return true;
        }

        function isRangeAllowed(start, end) {
            ...
            if (scope === 'day') {
                return start.toISOString().split('T')[0] === calendarPermission.reference_date;
            }

            if (scope === 'month') {
                return start.toISOString().slice(0, 7) === calendarPermission.reference_month;
            }

            if (scope === 'year') {
                return start.getFullYear() == calendarPermission.reference_year;
            }

            return true;
        }

        function loadEventCounts(start, end, permission_id) {
            $.ajax({
        ...
        function loadCountsForRange(start, end, mode, permission_id) {
            // EVENTS
            $.getJSON('<?= site_url("calendar/events_count"); ?>', { start, end, permission_id })
            .done(res => {
                if (res.success) updateEventsUI(res.data, mode);
            });

            // BRICKS
            $.getJSON('<?= site_url("calendar/bricks_count"); ?>', { start, end, permission_id })
            .done(res => {
        ...
        function renderCalendarModal(clickedDate){
            ...
            $.ajax({
                ...
                data: {
                    dateStr,
                    permission_id: calendarPermission.id
                },
                success: function (res) {
        ...
        function renderEvent(opt) {
            if (opt.height > 50) {
                return `
                    ...
                    data-permission-id=${opt.permission_id}
                    >
...
<script>
    ...
    $(document).on('click', '.event-item', function (e) {
        ...
        const $event_item = $(this);
        const timelineId = $event_item.data('timelineId');
        const permissionId = $event_item.data('permissionId') || '';

        let url = `<?=base_url('calendar/data-feeding-panel')?>?id=${timelineId}`;

        if (permissionId) {
            url += `&permission_id=${permissionId}`;
        }
