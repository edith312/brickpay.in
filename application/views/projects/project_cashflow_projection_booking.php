    ...
    <div class="row align-items-start">
        <div class="col-12">
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    <h5 class="card-title mb-3 text-primary fw-bold text-center">
                        Cash Flow Booking
                    </h5>
            ...
            <div id="calendarWrapper">
                ...
                <div id="calendarFilters" style="margin-bottom: 15px; text-align: right;">
                    ...
                    <button class="btn-filter">
                        <a class="me-auto text-light" href="<?= base_url("calendar/data-feeding-panel?project_id=$project_id") ?>">Data Feeding Panel</a>
                    </button>
                        ...
                        <div class="border border-black">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-around">
                                    <span>CashFlow: </span>
                                    <span class="" id="monthlyCashFlow">0</span>
                                </div>
                            </div>
                        </div>
                        <div class="border border-black">
                            ...
                            <div class="d-flex flex-column">
                                ...
                                <div class="d-flex justify-content-around">
                                    <span>CashFlow: </span>
                                    <span class="" id="yearlyCashflow">0</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div id="dayModal" class="modal">
    <div class="modal-content">
        ...
        <div class="d-flex justify-content-center align-items-center gap-1">
            ...
            <form class="d-flex mb-0" action="<?= base_url("calendar/data-feeding-panel?project_id=$project_id") ?>" id="timeForm" method="post">
                <!-- <div class="input-box">
                    <label> Opening </label>
                ...
                <div class="mx-2">
                    <button type="submit" class="btn btn-primary btn-sm" href="<?= base_url("calendar/data-feeding-panel?company_id=$company_id") ?>">Data Feeding Panel</button>
                </div>
            </form>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const mainCalendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventSources: [{
                    url: '<?= site_url("calendar/events_project?project_id=$project_id"); ?>',
                    method: "GET"
                },
                {
            ...
            datesSet: function(info) {
                try {
                    ...
                    loadEventCounts(info.startStr, info.endStr, project_id);

                    // 🔹 Monthly count = visible month range
                    loadCountsForRange(info.startStr, info.endStr, 'monthly', project_id);
                    ...
                    loadCountsForRange(yearStart, yearEnd, 'yearly', project_id);
        ...
        function loadEventCounts(start, end, project_id) {
            $.ajax({
                url: '<?= site_url("calendar/events_count_project"); ?>',
                type: 'GET',
                dataType: 'json',
                data: {
                    start: start,
        ...
        function loadCountsForRange(start, end, mode, project_id) {
            // EVENTS
            $.getJSON('<?= site_url("calendar/events_count_project"); ?>', { start, end, project_id })
            .done(res => {
                if (res.success) updateEventsUI(res.data, mode);
            });

            // BRICKS
            $.getJSON('<?= site_url("calendar/bricks_count_project"); ?>', { start, end, project_id })
            .done(res => {
        ...
        function updateEventsUI(rows, mode) {
            ...
            if (mode === 'monthly') $('#monthlyCashFlow').text(total);
            if (mode === 'yearly')  $('#yearlyCashFlow').text(total);
        }
...
<script>
    $(document).on('click', '.event-item', function (e) {
        ...
        window.location.href = `<?=base_url('calendar/data-feeding-panel')?>?id=${timelineId}&project_id=${project_id}`
    })
</script>
