            ...
            <div id="calendarWrapper">
                ...
                <div id="calendarFilters" style="margin-bottom: 15px; text-align: right;">
                    ...
                    <button class="btn-filter">
                        <a class="me-auto text-light" href="<?= base_url("calendar/data-feeding-panel?company_id=$company_id") ?>">Data Feeding Panel</a>
                    </button>
...
<div id="dayModal" class="modal">
    <div class="modal-content">
        ...
        <div class="d-flex justify-content-center align-items-center gap-1">
            ...
            <form class="d-flex mb-0" action="<?= base_url("calendar/data-feeding-panel?company_id=$company_id") ?>" id="timeForm" method="post">
                <!-- <div class="input-box">
                    <label> Opening </label>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const mainCalendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventSources: [{
                    url: '<?= site_url("calendar/events_company?company_id=$company_id"); ?>',
                    method: "GET"
                },
                {
            ...
            datesSet: function(info) {
                try {
                    ...
                    loadEventCounts(info.startStr, info.endStr, company_id);

                    // 🔹 Monthly count = visible month range
                    loadCountsForRange(info.startStr, info.endStr, 'monthly', company_id);
                    ...
                    loadCountsForRange(yearStart, yearEnd, 'yearly', company_id);
        ...
        function loadEventCounts(start, end, company_id) {
            $.ajax({
                url: '<?= site_url("calendar/events_count_company"); ?>',
                type: 'GET',
                dataType: 'json',
                data: {
                    start: start,
        ...
        function loadCountsForRange(start, end, mode, company_id) {
            // EVENTS
            $.getJSON('<?= site_url("calendar/events_count_company"); ?>', { start, end, company_id })
            .done(res => {
                if (res.success) updateEventsUI(res.data, mode);
            });

            // BRICKS
            $.getJSON('<?= site_url("calendar/bricks_count_company"); ?>', { start, end, company_id })
            .done(res => {
...
<script>
    $(document).on('click', '.event-item', function (e) {
        ...
        window.location.href = `<?=base_url('calendar/data-feeding-panel')?>?id=${timelineId}&company_id=${company_id}`
    })
</script>
